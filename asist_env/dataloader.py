...
Outputs:
    ...
    2) data/sparky_portals.csv -- cols = 'id', 'loc', 'connections' (room connections)
...
for i in sparky_rooms.index:
    ...
    df_sr = df_sr.append({'id': sparky_rooms['RoomID'][i], 'loc': midpt, 'connections': []}, ignore_index=True)
...
# Make portal data
df_sp = pd.DataFrame(columns=['id', 'loc', 'connections'])
for i in sparky_portals.index:
    ...
    df_sp = df_sp.append({'id': sparky_portals['Index'][i], 'loc': midpt, 'connections': []}, ignore_index=True)
...
for i in sparky_victims.index:
    df_sv = df_sv.append({'id':   sparky_victims['Index'][i], 
